...
class SES_Logger:
  ...
  def __init__(self, root):
    ...
    self.vars = ses.ses_data(root, self.NUM_OF_APS, self.menu)
    
    self.dim = "{}x{}".format(self.MIN_APP_WIDTH, self.MIN_APP_HEIGHT)
    self.root.geometry(self.dim)
    self.root.title(self.APP_TITLE)
    self.root.wm_attributes("-topmost",self.menu.return_Settings()['Theme']['on_top'])
  ...
  def create_SES_Form(self, centerFrame, col, row):
    ...
    core_FrameInner = tk.Frame(core_FrameLabel, bg= self.bg_theme, width= scrollBoxWidth, height= 70)
    core_FrameInner.grid_propagate(False)
    core_FrameInner.grid()
    
    #Both frames that are on top of the other, pretty tired
    core_Attributes_Frame = tk.Frame(core_FrameInner, bg= self.bg_theme)
    core_Attributes_Frame.grid(sticky= 'nsew', padx= 2)
    core_Form_Frame = tk.Frame(core_FrameInner, bg= self.bg_theme)
    core_Form_Frame.grid(sticky= 'nsew')
    

    
    ###
    core_Room_Dropbox = ttk.Combobox(core_Attributes_Frame, values= ["IDF", "MDF"], state= 'readonly')
    core_Room_Dropbox.config(width= 5, textvariable= self.vars.variables['core_Room'])
    core_Check = tk.Checkbutton(core_Attributes_Frame, text= "Complete", variable = self.vars.variables['core_Check'])
    core_Check.config(bg= self.bg_theme, activebackground= self.active_bg_theme)
    self.vars.variables['core_Check'].set(0)
    
    core_Room_Dropbox.grid(column= 0, row= 0)
    core_Check.grid(column= 1, row= 0, padx= 25)
    
    ###
    core_Port_Label = tk.Label(core_Form_Frame, text= 'Port:') 
    core_MAC_Label = tk.Label(core_Form_Frame, text = 'MAC:')
    
    core_Port = tk.Entry(core_Form_Frame, width = 4, textvariable= self.vars.variables['core_port'])
    self.vars.assign_Widget_From_SES_Logger('core_port', core_Port)
    core_MAC = tk.Entry(core_Form_Frame, width = 16, textvariable= self.vars.variables['core_mac'])
    ...
    #Assigns the combobox control over the AP variables after switching
    self.vars.assign_Widget_From_SES_Logger('core_Room', core_Room_Dropbox)
  ...
  def create_Textbox_Frames(self, centerFrame, col, row):
    ...
    notes = scrolledtext.ScrolledText(note_Frame, width= 47, height= 12, padx= 5, wrap=tk.WORD)
    notes.grid()
    ...
    #place frames 
    button_Frame.grid(column= 0, row= 0, sticky='nsew', pady= 2)
    note_Frame.grid(column= 0, row= 1, sticky='nsew', pady= 2)
    formatted_Frame.grid(column= 0, row= 2, sticky= 'nsew', pady= 5)
    
    
  def create_Textbox_Buttons(self, button_Frame):
    
    
    #release_Button = releaseCode_Button(button_Frame, self.menu.return_Settings()['Theme'], self.vars, col= 0, row= 0 )
    release_Button_And_Field(button_Frame, self.menu.return_Settings()['Theme'], self.vars, col= 0, row= 0 )
    pass
...
class release_Button_And_Field(tk.Frame):
  def __init__(self, root, theme, data, col= 0, row= 0, *args, **kw):
    
    tk.Frame.__init__(self, root, *args, **kw)
    
    def create_Button():
      self.release_Button = tk.Button(root, text = 'Release Code')
      self.release_Button.grid(column= col, row= row, sticky= 'nsew')
      ...
      def config_Style(reset = False):
        ...
        self.release_Button.config(activebackground= self.button_active_bg, activeforeground= self.button_active_fg)
        self.release_Button.config(bg= self.button_bg, fg= self.button_fg)
    ...
    def create_Entry():   #Maybe add ability to change theme later
      self.release_Entry = tk.Entry(root, width = 12)
      self.release_Entry.grid(column= col+1, row= row, sticky= 'nsew', padx= 3)
      
      data.assign_Widget_From_SES_Logger('release', self.release_Entry)
      def set_Style():    #Nothing yet
        pass
      
    def bind_Button():
      self.release_Button.config(command= data.copy_Release)
