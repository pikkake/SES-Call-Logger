...
class MenuBar:
  ...
  """
  ###############################################  
  SETTINGS_HEIGHT = 440      #Height of the settings window  357
      ...
      for selected_Data in setting_Data:
        ...
        for widget in widget_Dict:
          if selected_Data == widget:
            try:    #Normal Entry fields
              final[setting][selected_Data] = widget_Dict[widget].get().strip()
            except AttributeError:    #If it isn't a widget with .get()

              if widget_Dict[widget].instate(['selected']):
                final[setting][selected_Data] = 1
              else:
                final[setting][selected_Data] = 0
...
class theme_Settings(base_settings):
  ...
  def createForm(self):
    ...
    on_Top_Frame = tk.Frame(self.form)
    on_Top_Frame.grid(column= 0, row= 1)
    ...
    on_top = ttk.Checkbutton(on_Top_Frame, text= 'Always on top?', variable= self.on_top, name= 'on_top')
    entry_List.append(on_top)
    on_top.grid(sticky= 'nsew')
